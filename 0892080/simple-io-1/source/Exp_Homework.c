...
void Exp_1_Homework_B(void) {
	u16 sw;
	u8 key_pressed = FALSE;
	u16 present_led = BARLED1;
	short int led_state = 0x0001;

	//writeb_virtual_io(BARLED1, led_state);
	writeb_virtual_io(present_led, led_state);
		...
		if ((key_pressed == FALSE) && ((sw & KEY_L) || (sw & KEY_R))) {
			...
			if (sw & KEY_L) {
				...
				if (!(led_state == 0x0080))
					...
					if ((led_state == 0x0080) && (present_led == BARLED1)) { //If it is max left and state of LED is pressed 'KEY_L'
						writeb_virtual_io(present_led, 0);
						led_state = 0x0001;
						present_led = BARLED2;
					}

					if ((led_state == 0x0080) && (present_led == BARLED2)) { //If it is max right and state of LED is pressed 'KEY_R'
						writeb_virtual_io(present_led, 0);
						led_state = 0x0001;
						present_led = BARLED1;
					}
			...
			if (sw & KEY_R) {
				...
				if (!(led_state == 0x0001))
					...
					if ((led_state == 0x0001) && (present_led == BARLED2)) { //If it is max right and state of LED is pressed 'KEY_R'
						writeb_virtual_io(present_led, 0);
						led_state = 0x0080;
						present_led = BARLED1;
					}

					if ((led_state == 0x0001) && (present_led == BARLED1)) { //If it is max left and state of LED is pressed 'KEY_L'
						writeb_virtual_io(present_led, 0);
						led_state = 0x0080;
						present_led = BARLED2;
					}
