...
wifi_connect(void)
{
	...
	if(PA_InitServer(&listen_socket,3389,PA_NONBLOCKING_TCP,10) <  0 )
	{
	...
	FD_SET(data_socket, &dset);

	tv.tv_sec = 0;
	tv.tv_usec = 100000;

	if(select(listen_socket+1, &rset, NULL, NULL, &tv) <= 0 || !FD_ISSET(listen_socket, &rset)) {
		;
	}
	// Set Nonblocking Mode
	//    if ((ret = ioctl(listen_socket, FIONBIO, &one)) < 0) {
	//        confirm_error("wifi: Listen_Socket Non-blocking Mode Failed", 0);
	//        iprintf("wifi: Listen_Socket Non-blocking Mode Failed\n");
	...
	while (1) {
		...
		if (errno != EWOULDBLOCK) {
			...
			PA_OutputText(1,10,20,"wifi: accept error code:%d\n", data_socket);
			confirm_error("Connection Failed", 0);
			iprintf("Connection Failed %d~~~~~~~~~~~~~~~\n",errno);
			ret = DSH_CONNECTION_FAILED;
			return 0;
			goto leave;
		}
	...
	if(select(data_socket+1, &dset, NULL, NULL, &tv) <= 0 || !FD_ISSET(data_socket, &dset)) {
		ret = 0;
	}
