...
vStartExpTasks(void)
{
	...
	// 여기에 필요한 수 만큼 Semaphore 초기화 (vSemaphoreCreateBinary) <--------
...
static portTASK_FUNCTION(Ball_Task, pvParameters)
{
	...
	// 여기에 각  Ball의 초기 위치 (x,y) 설정 <-------- 지난 실험 내용

	while(1) {

		// 여기에 각 Ball이 교차점에 있고, R Key가 눌렸다면  Semaphore take <--------

		draw_my_box(prevX, prevY, COLOR_BLACK);	// 예전 위치 삭제
		draw_my_box(x, y, p->color);			// 새 위치에 그림
		vTaskDelay(MSEC2TICK(p->delay));		// Delay

		// 여기에 각 Ball이 교차점에 있으면, Semaphore Give <--------
