...
int main(int argc, char* argv[]) {
	...
	nds_sin.sin_family = AF_INET;
	nds_sin.sin_addr.s_addr = inet_addr("192.168.0.2");
	//nds_sin.sin_addr.s_addr = htonl(INADDR_ANY);
	nds_sin.sin_port = htons(Port);
	...
	while (1) {
		...
		if (pid < 0) {
			...
			if (read(clnt_socket, (struct header *) &FHeader, sizeof(FHeader))
				...
				/*
			strcpy(fname, "./");
			strcat(fname, tmp_port);
			strcat(fname, "/");
			strcat(fname, FHeader.filename);
			*/
			//strcpy(fname, FHeader.filename);
			//sprintf(fname, "./%s/%s", tmp_port, FHeader.filename);
