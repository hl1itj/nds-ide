...
TestSerial(void)
{
    ...
    while (1) {
        if (_kbhit()) {
            ch = _getch();
            WriteFile(S_Port, &ch, 1, &size, NULL);
            if (size != 1)
                printf("(%d)", size);
        }
        ReadFile(S_Port, &ch, 1, &size, NULL);
        if (size == 1)
            printf("%c", ch);
        //else
        //    printf("[%d]", size);
    ...
    while (count) {
        switch (Channel) {
          case CHANNEL_TCP :
            ...
            break;
          case CHANNEL_SERIAL :
            if (!(ret = WriteFile(S_Port, buf, count, &size, NULL))) {
				goto loop;
				fprintf(stderr, "\nSize : %d\n", size);
    ...
    while (count) {
        switch (Channel) {
          case CHANNEL_TCP :
            ...
            break;
          case CHANNEL_SERIAL :
            if (!ReadFile(S_Port, buf, count, &size, NULL)) {
...
recv_data_byte(unsigned char *buf, int count)
{
    ...
    while (count) {
		...
		case CHANNEL_TCP :
            ...
            break;
		case CHANNEL_SERIAL :
			ReadFile(S_Port, buf, read_size, &size, NULL);
            break;
		case CHANNEL_USB :
            //write_to usb channel();
            break;
